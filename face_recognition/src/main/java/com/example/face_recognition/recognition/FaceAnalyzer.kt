...
internal class FaceAnalyzer(
    ...
    private var heigtScaleFactor = 1F
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        ...
        if (face != null) {
            ...
            } else if (detectStatus == FaceAnalyzerStatus.LeftWink && (face.leftEyeOpenProbability
                    ?: 0F) < EYE_SUCCESS_VALUE && (face.rightEyeOpenProbability
                    ?: 0F) > EYE_SUCCESS_VALUE
            ) {
            ...
            listener?.notDetect()
            listener?.detectProgress(0F, "얼굴인식에 실패했습니다.\n처음으로 돌아갑니다.")
        }
    }
    ...
    override fun analyze(image: ImageProxy) {
        ...
        heigtScaleFactor = preview.height.toFloat() / image.width
        detectFace(image)
    }
    ...
    private fun Int.translateY() = toFloat() * heigtScaleFactor
