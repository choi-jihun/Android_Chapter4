...
import com.example.chapter4_3.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
        ...
        with(binding) {
            when (root.currentState) {
                R.id.fanOut -> {
                    ...
                    collapsedCardCompleteListener(endStateId)
                }
                R.id.thirdCardOnTop -> {
    ...
    private fun collapsedCardCompleteListener(@IdRes endStateId: Int) {
        with(binding.root) {
            setTransitionListener(object : TransitionAdapter() {
                override fun onTransitionCompleted(motionLayout: MotionLayout?, currentId: Int) {
                    if (currentId == R.id.firstCardOnTop) {
                        setTransition(R.id.firstCardOnTop, endStateId)
                        transitionToEnd()
                    }
